...
> **目标读者**：熟悉 Java 的工程师，希望快速理解 StarRocks BE（C++）中基础设施层各目录与文件的职责。
>
...
本批共涵盖 **7 个目录**，约 181 个源文件。

| 目录 | 文件数 | 定位 | Java 类比 |
|------|--------|------|-----------|
| `common/` | ~33 | 全局公共基础设施（配置、错误处理、日志、进程管理等） | `java.lang` + Spring 配置 |
| `types/` | ~20 | 数据类型定义（逻辑类型、日期、Bitmap、HLL 等） | `java.sql.Types` + 自定义值类型 |
| `column/` | ~60 | 向量化列式内存数据结构（核心数据容器） | 类似 Arrow 的 `FieldVector` |
| `serde/` | ~6 | 列数据的序列化/反序列化 | `Serializable` + Protobuf |
| `simd/` | ~6 | SIMD 向量化加速原语 | 无直接对应，底层 CPU 指令优化 |
| `io/` | ~29 | 字节流抽象（输入/输出/可寻址流） | `java.io.InputStream/OutputStream` |
| `fs/` | ~27 | 文件系统抽象层（POSIX/S3/HDFS/Broker 等） | `java.nio.file.FileSystem` |
...
| C++ 概念 | 说明 | Java 类比 |
|----------|------|-----------|
| RAII (Resource Acquisition Is Initialization) | 对象构造时获取资源，析构时释放。`ClosureGuard`、智能指针均基于此 | try-with-resources / `AutoCloseable` |
| `std::unique_ptr<T>` | 独占所有权智能指针 | 无直接对应（GC 自动管理），概念类似 `AutoCloseable` 资源 |
| `std::shared_ptr<T>` | 共享所有权智能指针（引用计数），如 `ChunkPtr = shared_ptr<Chunk>` | 无直接对应（Java 的 GC 隐式引用计数） |
| `std::variant` | 类型安全的联合体，`Datum` 类用此持有多种数据类型 | `Object`（但 `variant` 编译期类型安全） |
| 模板特化 (`template<>`) | 为特定类型提供专门实现，如 `FixedLengthColumn<int32_t>` | 泛型特化（Java 不支持，只有类型擦除） |
| 宏展开 (`APPLY_FOR_ALL_*`) | 编译期代码生成，为每种类型生成模板特化 | 代码生成 / APT |
| `constexpr` | 编译期常量计算，如 `DEFAULT_CHUNK_SIZE` | `static final`（但 `constexpr` 可计算复杂表达式） |
| `virtual` / 纯虚函数 | 虚函数 / 抽象方法，如 `Column::clone()` | `abstract` 方法 / `interface` |
| Visitor Pattern | 访问者模式，`ColumnVisitor` 对不同列类型做分派 | Visitor 设计模式（用法一致） |
| `#ifdef __AVX2__` | 条件编译，按 CPU 指令集选择不同实现 | Java 无直接对应（JVM 自动做 JIT 优化） |
| `StatusOr<T>` | 要么返回成功值，要么返回错误状态。几乎所有 IO 操作的返回值 | `Optional<T>` + 异常，或 Vavr `Either<Error, T>` |
